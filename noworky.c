...
{
    //The Bug was here: It was doing "unsigned int i;" but should be doing "int i".
    //Unsigned int i was a problem since when i goes to 0 and it does i-- it goes to...
	// ... a randome number since it is unsigned and can't go less then 0...
	// ... Hence the loop still runs with a very high i and causes SEG FAULT

	int i; //Correction done 
    // unsigned int i; //original code
	for (i = size-1; i >= 0; i--) {
